...
<!-- source-sha256: 7079615033718b8d -->
...
- **What STE-54 found about `predicted_starter` still holds; what the engine does with rotation no longer does.** Evaluated on 8 Sep against the whole live feed, it is a hard-constrained XI — exactly eleven per club, binary, with no confidence attached — so it cannot price the 11th-versus-12th case the rotation input exists for, and it would double-count availability. **That finding is unaffected by anything since.** What has changed is that rotation is no longer a multiplier at all (ruled 10 Sep, STE-60): effective points is the feed's projection taken whole, so this bullet's former conclusion — *rotation stays model-judged and the engine spec drops nothing* — no longer describes the engine. **Rotation is now gone from the engine altogether (ruled 10 Sep, STE-60; Decision Log #74) — and for a different reason than STE-54 was weighing.** STE-54 asked whether the feed's `predicted_starter` was good enough to *buy* the rotation input instead of judging it, and answered no. The engine no longer has a rotation input to buy or judge: the feed's projection already prices expected starting, so a rotation-risky player carries a low projection and loses the comparison unaided. **STE-54's finding is not overturned, it is bypassed** — the column still cannot price rotation, and nothing now needs it to. Read `predicted_starter = false` on an available player as a fact shown on the card, which is what STE-54 recommended and remains true. Two hazards recorded with STE-54 and still live: `xi_known` is `true` for all 654 rows and carries no information, and a blanking club still shows eleven starters.
...
- **Slice 4's criteria are written before its worked example is run — both, in that order** (ruled 9 Sep, recorded on STE-60). `ENGINE.criteria.md` carried zero `AC-` identifiers, so baseline criteria went into PRD 3.2 and the file was regenerated first; the worked example is run and iterated against them. **Done 10 Sep** — it now declares `ENGINE-AC-01` to `ENGINE-AC-06`. The coverage script was taught that identifier shape in the same change: its pattern recognised only `F1`–`F9`, and the engine is not a feature and has no number, so an engine criterion would have been **invisible to the report** rather than merely uncovered. **The order is the substance, not a preference.** Run the example first and whatever it produces becomes the standard by default, with nothing left to disagree with it. The criteria are **baseline, not exhaustive** — the PRD caps criteria at five per feature, and the arithmetic has more cases than that; the detail stays in the worked example and the unit tests, while the criteria carry the claims that must not silently change.
  **And the pencil check the order exists to make possible then ran, closing Decision Log #25.** Against **GW4, on the real squad, with both feeds live** — not a template and not a hypothetical week, a distinction that changed the answer once already: the first pass used the most-owned fifteen, which carries three £4.0m defenders and therefore flatters every transfer figure. Outcome: **transfer stays 2.0, substitution 0.5, the noise floor 20; captain and vice move from 0.8 to 0.5.** The armband's move is a consistency ruling rather than a tuning one — the ceiling tie-break had already taken over the variance job the higher figure was doing, and a +2.80 vice call reading *lean* beside a +3.00 substitution reading *strong* is the same edge charged differently for its label. **Nothing was fitted to outcomes** — one worked week, judged against whether a call you would actually make reads the way it should.
- **STE-87 and STE-88 block slice 5.** Both surfaced writing the architecture spec, and both are questions the F3 build would otherwise default past rather than ask: where purchase prices are read from, and what the two probabilities in the vice and bench-order arithmetic actually are. Answer them Thursday, not during Friday's slice.
  **Both were answered on Thursday and both are closed — the constraint did its job, and these are the answers slice 5 opens with.**
  **STE-87 · where purchase prices come from.** The public `picks/` endpoint carries **neither** purchase nor selling price, so none of the ticket's three fallbacks was taken and a fourth option it had not considered turned out to be exact: purchase price is recovered from **`entry/{id}/transfers/`'s `element_in_cost`**, falling back to **`now_cost − cost_change_start`** for a player held since gameweek 1. **Free Hit gameweeks are skipped** — the squad reverts afterwards, so transfers recorded against one were never really made and name players no longer owned; Wildcard weeks need no special handling. One extra public read plus arithmetic on data already in hand, and **that work lands inside slice 5**. Decision Log #83.
  **STE-88 · the two probabilities.** **Neither exists, and neither is replaced by an estimate.** The spec's reading of both as `1 − availability` died with the availability multiplier, and inventing a figure would be the double-count that removed it. The rule instead: *where the arithmetic names a probability no source publishes, the multiplier is dropped.* So the captain is the highest eligible projection, the vice the second-highest, the outfield bench is ordered by the same figure with excluded players last, and each net is the **plain difference**. Now stated as **`ENGINE-AC-06`**; Decision Log #80 and #81.
